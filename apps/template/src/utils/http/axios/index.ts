...
import {ConfigEnum, ContentTypeEnum, RequestEnum, ResultEnum, UrlPrefix,} from '/@/enums/httpEnum';
...
//todo deepMerge
import {deepMerge, isString, MD5HttpSignUtils} from '@dfsj/utils';
        ...
        deepMerge(
            {
                ...
                authenticationScheme: getAuthCache<LoginInfo | {}>(LOGIN_INFO_KEY)?.token_type ?? 'Bearer',
...
//FIXME 通用的POST请求参数，减少每个api里面重复书写defHttp
export const unifyPostHttp = (url: string, params = {}, config: Partial<RequestOptions> = {}) => {
    return defHttp.post(
        {
            url: url,
            params: params,
