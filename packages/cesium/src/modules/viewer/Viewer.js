...
import {LayerEventType, LayerGroupEventType, MouseEvent, SceneEvent, ViewerEvent} from '../event'
import {CameraOption, ViewerOption} from '../option'
import {DomUtil, Util} from '../utils'
import {Transform} from '../transform'
import createWidgets from '../widget'
import createTools from '../tools'
import {BaseLayerPicker, CesiumViewer} from '../exts'
import ImageryManager from "../imagery/ImageryManager";
...
class Viewer {
	constructor(id, options = {}) {
		 ...
		 * 影像图层管理器
		 */
		this._imageryManager = new ImageryManager(this)
	...
	get cameraPosition() {
		let position = Transform.transformCartographicToWGS84(this.camera.positionCartographic)
		if (position) {
	 ...
	 */
	getExtent() {
		const extend = this.camera.computeViewRectangle();
		const xMax = Cesium.Math.toDegrees(extend.east);
		const yMax = Cesium.Math.toDegrees(extend.north);
		const xMin = Cesium.Math.toDegrees(extend.west);
		const yMin = Cesium.Math.toDegrees(extend.south);
	 ...
	 * @returns {Viewer}
	 */
	flyTohpr(target, duration ,offset) {
		if (!Object.keys(offset).length) throw Error('offset is not defined');
		return this._delegate.flyTo(target?.delegate || target, {
			duration,
			offset
		})
	}
