<script lang="tsx">
import {ElCollapseTransition, ElTooltip, ElRow, ElIcon, ElCol} from 'element-plus';
...
const prefixCls = `ec-descriptions-component`
const defaultData = '--'
          ...
          >
            {props.title ? (
                <div
                    class={[
                      `${prefixCls}-header`,
                      `${prefixCls}-header__${props.size}`,
                      'relative  flex justify-between items-center layout-border__bottom px-10px cursor-pointer'
                    ]}
                    onClick={toggleClick}
                >
                          ...
                          {props.direction === 'horizontal' ? (
                              <div
                                ...
                                <div
                                  ...
                                  {item.slots?.prepend && item.slots?.prepend?.(get(props.data, item.field), props.data)}
                                  {item.slots?.default
                                      ? item.slots?.default(props.data)
                                      : get(props.data, item.field) ?? defaultData}
                                  {item.slots?.append && item.slots?.append?.(get(props.data, item.field), props.data)}
                                </div>
                              </div>
                          ) : (
                              <div
