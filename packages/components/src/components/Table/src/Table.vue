...
import {computed, CSSProperties, defineComponent, onMounted, PropType, ref, unref, watch,} from 'vue';
          ...
          if (v.type === 'index') {
            ...
            const columSlots = {
              default: (...args: any[]) => {
                ...
                //判断是否渲染自定义的
                const _slots = getSlot(slots, v.field, {
                  row: data.row,
                  field: v.field,
                  index: data.$index,
                });
            ...
            if (getSlot(slots, `${v.field}-header`)) {
              columSlots['header'] = (...args: any[]) => {
                const data = args[0];
                const _slots = getSlot(slots, `${v.field}-header`, {
                  row: data.row,
                  field: v.field,
                  index: data.$index,
                });
            ...
            if (props?.slots?.header) {
              columSlots['header'] = (...args: any[]) => props.slots?.header(...args);
            }
