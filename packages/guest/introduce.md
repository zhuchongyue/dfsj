...
**用于外部系统Iframe调用内部系统时，校验直接免登录系统的sdk**
...
| 贵阳市智慧防汛系统<br/>http://gywfc.dfsjcloud.com:8081/free/login<br/>http://gywfc.dfsjcloud.com:8081/free/valid/" + token<br/>登录账户/密码<br/>gydshare/gydashare520100                                                                     |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 注意事项：由于前台直接请求获取token会存在跨域问题，所以将获取登录token的接口放到前台的后台进行转发，自己设置路由进行调用，<br/>这里的示例是调用本地项目的/getToken路由地址进行的post请求,将获取到的token值与路由拼接 http://gywfc.dfsjcloud.com:8081/free/valid/+token<br/>然后进行跳转后验证登录，本示例使用的是基于nodejs的express.js web框架 |
...
<body>
<div id="toyj">
    <button onclick="init()">转到系统</button>
</div>
<div id="txt"></div>
<script>
    function init() {
        $("#txt").empty();
        $("#txt").append("开始登录获取token...");
        var obj = {username: "uid", password: "pwd"};
        $.ajax({
            type: 'post',
            dataType: 'json',
            ...
            success: function (res, status) {
                console.log(res);
                $("#txt").empty();
                $("#txt").append("开始验证token...");
                if (res.flag) {
                    window.location.href = "http://gywfc.dfsjcloud.com:8081/free/valid/" + res.result.data;
                }
            }
        })
    }
...
router.post('/', function (req, res) {
    var url = 'http://gywfc.dfsjcloud.com:8081/free/login';
    var params = {username: req.body.username, password: req.body.password};
    re(url, params, function (err, data) {
        console.log("返回登录token")
        console.log(data)
        res.json(data)
    })
});

function re(url, params, callback) {
    var options = {
        uri: url,
        method: 'POST',
        json: params,
    ...
    request(options, function (error, response, body) {
        if (!error && body != null && response.statusCode == 200) {
            callback(null, body);
        } else {
            //错误才记录信息
            logger.writeErr("options:" + JSON.stringify(options));
            logger.writeErr("body:" + JSON.stringify(body));
            callback(error, null);
        }
    });
}
...
import {GuestOptions, GuestResult, Signature} from "./type";

/***
 * 默认的配置信息
 */
export const DEFAULT_OPT: GuestOptions = {
 ...
 */
export const publicKeyEncryption = (key, value) => {
    const pub = JSrsasign.KEYUTIL.getKey(key);
    const enc = JSrsasign.KJUR.crypto.Cipher.encrypt(value, pub);
...
export default class Guest {
    ...
    async validate(params: Partial<Signature & GuestOptions>) {
        if (!Reflect.has(this.options, 'guestValidateUrl')) throw Error('验证地址错误，guestValidateUrl参数必传！');
        const {publicKey, username, signStr, role, digest, router, guestValidateUrl} = params;
        const sign4Java = new JSrsasign.KJUR.crypto.Signature({
            alg: "SHA256withRSA",
            prvkeypem: publicKey
        });
        sign4Java.updateString(params.appId + Guest.SUFFIX);
        //验证是否通过
        const isSignature = sign4Java.verify(JSrsasign.b64utohex(signStr));
    ...
    async handleReceiveMessageListener(event: any | EventListener) {
        ...
        if (event.data && event.data.type === 'iframe'
            && event.data.username
            && event.data.publicKey && event.data.appId && event.data.signStr) {
            try {
