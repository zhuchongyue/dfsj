import {BaseOptions, Cancel, FetchService, Result, Run, Service} from './types'
import {ref, shallowRef, watch} from 'vue'
import {debounce, throttle, throttleAndDeBounce} from '../utils'
import {clearLoadingDelayTimer, loadingDelayAsync} from './src/loadingDelay'
import {argsSymbolKey, serveiceProxy} from './src/service'
...
const useRequest = <T>(service: Service | FetchService, options?: BaseOptions) => {
	...
	let run: Run = (...args: any[]) => {
		...
		serveiceProxy(service, args, reqCache)
			.then((responseData) => {
